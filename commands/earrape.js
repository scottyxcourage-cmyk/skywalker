...
module.exports = async (sock, chatId, message) => {
    try {
        ...
        const audMsg = quoted?.audioMessage; if (!audMsg) return reply(sock, chatId, "❌ Reply to an audio with .earrape", message);
        await reply(sock, chatId, "🔥 Earrape effect...", message);
        const stream = await downloadContentFromMessage(audMsg, "audio");
        const chunks = []; for await (const c of stream) chunks.push(c);
        const tmpIn=path.join("./temp",`er_in_${Date.now()}.ogg`), tmpOut=path.join("./temp",`er_out_${Date.now()}.mp3`);
