...
module.exports = async (sock, chatId, message, args) => {
    const url = args[0]?.trim(); if (!url||!url.startsWith("http")) return reply(sock, chatId, "❌ Usage: .ssweb <URL>", message);
    await reply(sock, chatId, "📸 Taking screenshot...", message);
    try {
    ...
    } catch { await reply(sock, chatId, "❌ Screenshot failed.", message); }
};