...
module.exports = async (sock, chatId, message, args) => {
    const url = args[0]?.trim(); if (!url||!url.includes("facebook")) return reply(sock, chatId, "❌ Usage: .facebook <url>", message);
    await reply(sock, chatId, "📘 Downloading Facebook...", message);
    try {
        ...
        if (!match) return reply(sock, chatId, "❌ Could not download. Make sure video is public.", message);
