...
module.exports = async (sock, chatId, message) => {
    try {
        ...
        const audMsg = quoted?.audioMessage; if (!audMsg) return reply(sock, chatId, "❌ Reply to an audio with .tovideo", message);
        await reply(sock, chatId, "🎬 Converting...", message);
        const stream = await downloadContentFromMessage(audMsg, "audio");
        const chunks = []; for await (const c of stream) chunks.push(c);
        const tmpIn=path.join("./temp",`tv_in_${Date.now()}.ogg`), tmpOut=path.join("./temp",`tv_out_${Date.now()}.mp4`);
