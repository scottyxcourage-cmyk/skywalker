...
async function autoReplyCommand(sock, chatId, message, args) {
    ...
    if (sub==="set") { const rest=args.slice(1).join(" "); const parts=rest.split("|").map(s=>s.trim()); if(parts.length<2) return reply(sock,chatId,"❌ Usage: .autoreply set <trigger> | <response>",message); if(!d.rules)d.rules={}; d.rules[parts[0].toLowerCase()]=parts[1]; save(d); return reply(sock,chatId,`✅ Set! Trigger: ${parts[0]}`,message); }
}
