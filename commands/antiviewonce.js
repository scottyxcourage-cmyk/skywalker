...
module.exports = async (sock, chatId, message, args) => {
    ...
    if (!sub) return reply(sock, chatId, `👁️ *Anti-ViewOnce*\nStatus: ${d.enabled?"✅ ON":"❌ OFF"}\n\n.antiviewonce on/off`, message);
    if (sub==="on") { save({enabled:true}); return reply(sock, chatId, "✅ Enabled! View-once media will be auto-revealed.", message); }
    if (sub==="off") { save({enabled:false}); return reply(sock, chatId, "❌ Disabled.", message); }
};