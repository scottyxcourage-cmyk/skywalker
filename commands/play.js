...
module.exports = async (sock, chatId, message, args) => {
    const q = args.join(" ").trim(); if (!q) return reply(sock, chatId, "❌ Usage: .play <song name>", message);
    ...
    try {
        ...
        if (vid.duration.seconds>600) return reply(sock, chatId, "❌ Too long (max 10 min).", message);
