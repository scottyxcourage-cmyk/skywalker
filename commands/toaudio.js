...
module.exports = async (sock, chatId, message) => {
    try {
        ...
        const vidMsg = quoted?.videoMessage; if (!vidMsg) return reply(sock, chatId, "❌ Reply to a video with .toaudio", message);
        await reply(sock, chatId, "🎵 Extracting audio...", message);
        const stream = await downloadContentFromMessage(vidMsg, "video");
        const chunks = []; for await (const c of stream) chunks.push(c);
        const tmpIn=path.join("./temp",`ta_in_${Date.now()}.mp4`), tmpOut=path.join("./temp",`ta_out_${Date.now()}.mp3`);
        fs.writeFileSync(tmpIn, Buffer.concat(chunks));
        execSync(`ffmpeg -i "${tmpIn}" -vn -acodec mp3 "${tmpOut}" -y`);
        await sock.sendMessage(chatId, { audio: fs.readFileSync(tmpOut), mimetype: "audio/mpeg" }, { quoted: message });
