...
module.exports = async (sock, chatId, message) => {
    try {
        ...
        const audMsg = quoted?.audioMessage; if (!audMsg) return reply(sock, chatId, "❌ Reply to an audio with .robot", message);
        await reply(sock, chatId, "🤖 Robot effect...", message);
        const stream = await downloadContentFromMessage(audMsg, "audio");
        const chunks = []; for await (const c of stream) chunks.push(c);
        const tmpIn=path.join("./temp",`r_in_${Date.now()}.ogg`), tmpOut=path.join("./temp",`r_out_${Date.now()}.mp3`);
