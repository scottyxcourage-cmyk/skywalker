...
module.exports = async (sock, chatId, message) => {
    ...
    for (const user of mentioned) {
        ...
        if (w[key] >= max) {
            await sock.sendMessage(chatId, { text: `⛔ @${user.split("@")[0]} got *${w[key]}/${max}* warnings — kicked!${SIG}`, mentions: [user] });
            try { await sock.groupParticipantsUpdate(chatId,[user],"remove"); } catch {}
            w[key]=0; saveW(w);
        } else {
            await sock.sendMessage(chatId, { text: `⚠️ *Warning ${w[key]}/${max}*\n@${user.split("@")[0]} watch yourself!${SIG}`, mentions: [user] }, { quoted: message });
        }
    }
};